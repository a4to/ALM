...
const logger = winston.createLogger({
  transports: [
    ...
    new winston.transports.File({
      filename: path.join(app.getPath('home'), '.alm', 'logs', 'server.log'),
...
function server() {
  const binary = app.isPackaged
    ? path.join(process.resourcesPath, 'alm')
    : path.resolve(process.cwd(), '..', 'alm')
...
const updateURL = `https://alm.com/api/update?os=${process.platform}&arch=${
  process.arch
