...
func TestParseNameParts(t *testing.T) {
	cases := []struct {
		...
		{
			in: "registry.alm.ai/library/dolphin-mistral:7b-v2.6-dpo-laser-q6_K",
			want: Name{
				Host:      "registry.alm.ai",
				Namespace: "library",
				Model:     "dolphin-mistral",
				Tag:       "7b-v2.6-dpo-laser-q6_K",
			},
			wantFilepath: filepath.Join("registry.alm.ai", "library", "dolphin-mistral", "7b-v2.6-dpo-laser-q6_K"),
		},
		{
		...
		},
		{
			...
			wantFilepath: filepath.Join("registry.alm.ai", "namespace", "model", "latest"),
		},
		{
			in: "model",
			want: Name{
			...
			wantFilepath: filepath.Join("registry.alm.ai", "library", "model", "latest"),
		},
		{
			in: "h/nn/mm:t",
			want: Name{
...
func TestNameparseNameDefault(t *testing.T) {
	...
	n := ParseName(name)
	got := n.String()
	want := "registry.alm.ai/library/xx:latest"
	if got != want {
...
func TestDisplayShortest(t *testing.T) {
	cases := map[string]string{
		"registry.alm.ai/library/model:latest": "model:latest",
		"registry.alm.ai/library/model:tag":    "model:tag",
		"registry.alm.ai/namespace/model:tag":  "namespace/model:tag",
