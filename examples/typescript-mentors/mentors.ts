...
async function getSystemPrompt(mentor: Mentor, isLast: boolean, question: string): Promise<string> {
  ALM.setModel(getMentorFileName(mentor));
  const info = await ALM.showModelInfo()
...
async function main() {
  ...
  for await (const mentor of mentors) {
    ...
    ALM.setModel(getMentorFileName(mentor));
    ALM.setSystemPrompt(SystemPrompt);
    let output = '';
    process.stdout.write(`\n${mentor.char}: `);
    for await (const chunk of ALM.streamingGenerate(theConversation + `Continue the conversation as if you were ${mentor.char} on the question "${question}".`)) {
      if (chunk.response) {
