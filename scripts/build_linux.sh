...
for TARGETARCH in ${BUILD_ARCH}; do
    ...
    docker cp builder-$TARGETARCH:/go/src/github.com/a4to/alm/alm ./dist/alm-linux-$TARGETARCH

    if [ "$TARGETARCH" = "amd64" ]; then
        docker cp builder-$TARGETARCH:/go/src/github.com/a4to/alm/dist/deps/ ./dist/
    fi
