...
import (
	...
	"github.com/a4to/alm/progress"
	"github.com/a4to/alm/server"
...
func tempZipFiles(path string) (string, error) {
	tempfile, err := os.CreateTemp("", "alm-tf")
	if err != nil {
		return "", err
	}
...
func errFromUnknownKey(unknownKeyErr error) error {
	...
	if len(matches) > 0 {
		...
		if runtime.GOOS == "linux" && serverPubKey != localPubKey {
			// try the alm service public key
			svcPubKey, err := os.ReadFile("/usr/share/alm/.alm/id_ed25519.pub")
			if err != nil {
				return unknownKeyErr
			}
		...
		msg.WriteString("\nAdd your key at:\n")
		msg.WriteString("https://alm.com/settings/keys")
...
func PushHandler(cmd *cobra.Command, args []string) error {
	...
	if err := client.Push(cmd.Context(), &request, fn); err != nil {
		...
		isOllamaHost := strings.HasSuffix(host, ".alm.ai") || strings.HasSuffix(host, ".alm.com")
		if strings.Contains(err.Error(), errtypes.UnknownOllamaKeyErrMsg) && isOllamaHost {
			// the user has not added their alm key to alm.com
...
func initializeKeypair() error {
	...
	privKeyPath := filepath.Join(home, ".alm", "id_ed25519")
	pubKeyPath := filepath.Join(home, ".alm", "id_ed25519.pub")
...
func checkServerHeartbeat(cmd *cobra.Command, _ []string) error {
	...
	if err := client.Heartbeat(cmd.Context()); err != nil {
		...
		if err := startApp(cmd.Context(), client); err != nil {
			return fmt.Errorf("could not connect to alm app, is it running?")
		}
	}
	return nil
}
