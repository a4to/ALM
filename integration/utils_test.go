...
func GetTestEndpoint() (*api.Client, string) {
	defaultPort := "11434"
	ALMHost := os.Getenv("OLLAMA_HOST")

	scheme, hostport, ok := strings.Cut(ALMHost, "://")
	if !ok {
		scheme, hostport = "http", ALMHost
	}
...
func startServer(t *testing.T, ctx context.Context, ALMHost string) error {
	// Make sure the server has been built
	CLIName, err := filepath.Abs("../ALM")
	if err != nil {
		return err
	}
	...
	if tmp := os.Getenv("OLLAMA_HOST"); tmp != ALMHost {
		slog.Info("setting env", "OLLAMA_HOST", ALMHost)
		t.Setenv("OLLAMA_HOST", ALMHost)
	}

	slog.Info("starting server", "url", ALMHost)
	done, err := lifecycle.SpawnServer(ctx, "../ALM")
	if err != nil {
...
func InitServerConnection(ctx context.Context, t *testing.T) (*api.Client, string, func()) {
	...
	if os.Getenv("OLLAMA_TEST_EXISTING") == "" {
		...
		fp, err := os.CreateTemp("", "ALM-server-*.log")
		if err != nil {
