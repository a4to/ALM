...
function init_vars {
    ...
    if (!$script:cmakeTargets) {
        $script:cmakeTargets = @("ALM_llama_server")
    }
    $script:cmakeDefs = @(
    ...
    $script:DIST_BASE = "${script:SRC_DIR}\dist\windows-${script:ARCH}\ALM_runners"
...
function apply_patches {
    ...
    if (!(Select-String -Path "${script:llamacppDir}/CMakeLists.txt" -Pattern 'ALM')) {
        Add-Content -Path "${script:llamacppDir}/CMakeLists.txt" -Value 'add_subdirectory(../ext_server ext_server) # ALM'
    }
...
function build_oneapi() {
  if ((-not "${env:OLLAMA_SKIP_ONEAPI_GENERATE}") -and ("${env:ONEAPI_ROOT}"))  {
    ...
    if ($null -ne $script:DUMPBIN) {
      & "$script:DUMPBIN" /dependents "${script:buildDir}/bin/ALM_llama_server.exe" | Select-String ".dll"
    }
    sign
    install
