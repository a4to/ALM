...
func (s *Scheduler) processPending(ctx context.Context) {
	for {
		...
		case pending := <-s.pendingReqCh:
			...
			// see https://github.com/a4to/ALM/issues/4165
...
func (s *Scheduler) load(req *LlmRequest, ggml *llm.GGML, gpus gpu.GpuInfoList, numParallel int) {
	...
	if err != nil {
		...
		if errors.Is(llm.ErrUnsupportedFormat, err) || strings.Contains(err.Error(), "failed to load model") {
			err = fmt.Errorf("%v: this model may be incompatible with your version of Ollama. If you previously pulled this model, try updating it by running `ALM pull %s`", err, req.model.ShortName)
		}
