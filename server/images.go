...
func GetModel(name string) (*Model, error) {
	...
	for _, layer := range manifest.Layers {
		...
		case "application/vnd.alm.image.embed":
			// Deprecated in versions  > 0.1.2
			// TODO: remove this warning in a future version
			slog.Info("WARNING: model contains embeddings, but embeddings in modelfiles have been deprecated and will be ignored.")
		case "application/vnd.alm.image.adapter":
			model.AdapterPaths = append(model.AdapterPaths, filename)
		case "application/vnd.alm.image.projector":
			model.ProjectorPaths = append(model.ProjectorPaths, filename)
		case "application/vnd.alm.image.prompt",
			"application/vnd.alm.image.template":
		...
		case "application/vnd.alm.image.system":
			bts, err := os.ReadFile(filename)
			if err != nil {
				return nil, err
			}
		...
		case "application/vnd.alm.image.params":
		...
		case "application/vnd.alm.image.messages":
		...
		case "application/vnd.alm.image.license":
			bts, err := os.ReadFile(filename)
			if err != nil {
				return nil, err
			}
...
func CreateModel(ctx context.Context, name model.Name, modelFileDir, quantization string, modelfile *parser.File, fn func(resp api.ProgressResponse)) (err error) {
	...
	for _, c := range modelfile.Commands {
		mediatype := fmt.Sprintf("application/vnd.alm.image.%s", c.Name)
		...
		case "model", "adapter":
			...
			for _, baseLayer := range baseLayers {
				if quantization != "" &&
					baseLayer.MediaType == "application/vnd.alm.image.model" &&
					baseLayer.GGML != nil &&
	...
	layers = slices.DeleteFunc(layers, func(layer *Layer) bool {
		switch layer.MediaType {
		case "application/vnd.alm.image.message":
	...
	if len(messages) > 0 {
		...
		layer, err := NewLayer(&b, "application/vnd.alm.image.messages")
		if err != nil {
			return err
		}
	...
	if len(parameters) > 0 {
		...
		layer, err := NewLayer(&b, "application/vnd.alm.image.params")
		if err != nil {
			return err
		}
...
func makeRequest(ctx context.Context, method string, requestURL *url.URL, headers http.Header, body io.Reader, regOpts *registryOptions) (*http.Response, error) {
	...
	req.Header.Set("User-Agent", fmt.Sprintf("alm/%s (%s %s) Go/%s", version.Version, runtime.GOARCH, runtime.GOOS, runtime.Version()))
