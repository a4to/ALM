...
func TestRequests(t *testing.T) {
	...
	scenario3a := newScenario(t, ctx, "alm-model-3a", 1*format.GigaByte)
	scenario3b := newScenario(t, ctx, "alm-model-3b", 24*format.GigaByte)
	scenario3c := newScenario(t, ctx, "alm-model-4a", 30)
	scenario3c.req.opts.NumGPU = 0                           // CPU load, will be allowed
	scenario3d := newScenario(t, ctx, "alm-model-3c", 30) // Needs prior unloaded
...
func TestGetRunner(t *testing.T) {
	...
	scenario1a := newScenario(t, ctx, "alm-model-1a", 10)
	scenario1a.req.sessionDuration = &api.Duration{Duration: 0}
	scenario1b := newScenario(t, ctx, "alm-model-1b", 10)
	scenario1b.req.sessionDuration = &api.Duration{Duration: 0}
	scenario1c := newScenario(t, ctx, "alm-model-1c", 10)
...
func TestAlreadyCanceled(t *testing.T) {
	...
	done2()
	scenario1a := newScenario(t, dctx, "alm-model-1", 10)
	scenario1a.req.sessionDuration = &api.Duration{Duration: 0}
	s := InitScheduler(ctx)
	slog.Info("scenario1a")
	s.pendingReqCh <- scenario1a.req
