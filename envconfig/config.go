...
func AsMap() map[string]EnvVar {
	ret := map[string]EnvVar{
		...
		"OLLAMA_HOST":              {"OLLAMA_HOST", Host, "IP Address for the alm server (default 127.0.0.1:11434)"},
...
func LoadConfig() {
	...
	if runtime.GOOS == "windows" && RunnersDir == "" {
		...
		for _, p := range paths {
			candidate := filepath.Join(p, "alm_runners")
		...
		if RunnersDir == "" {
			slog.Error("unable to locate llm runner directory.  Set OLLAMA_RUNNERS_DIR to the location of 'alm_runners'")
		}
	}
...
func getModelsDir() (string, error) {
	...
	return filepath.Join(home, ".alm", "models"), nil
}
