...
func PayloadsDir() (string, error) {
	...
	if payloadsDir == "" {
		...
		pidFilePath := filepath.Join(tmpDir, "ALM.pid")
		...
		// This will typically look like /tmp/ALM3208993108/runners on linux
...
func cleanupTmpDirs() {
	dirs, err := filepath.Glob(filepath.Join(os.TempDir(), "ALM*"))
	if err != nil {
		return
	}
	for _, d := range dirs {
		...
		raw, err := os.ReadFile(filepath.Join(d, "ALM.pid"))
		if err != nil {
			slog.Warn("failed to read ALM.pid", "path", d, "error", err)
		...
		if err == nil && !errors.Is(proc.Signal(syscall.Signal(0)), os.ErrProcessDone) {
			slog.Warn("found running ALM", "pid", pid, "path", d)
			// Another running ALM, ignore this tmpdir
			continue
		}
